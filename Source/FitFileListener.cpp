...
void FitFileListener::OnMesg(fit::Mesg &mesg) {
    printf("On Mesg:\n");
    std::wcout << L"   New Mesg: " << mesg.GetName().c_str() << L".  It has "
               << mesg.GetNumFields() << L" field(s) and "
               << mesg.GetNumDevFields() << " developer field(s).\n";
...
void FitFileListener::OnMesg(fit::FileIdMesg &mesg) {
    printf("File ID:\n");
    if (mesg.IsTypeValid())
        printf("   Type: %d\n", mesg.GetType());
...
void FitFileListener::OnMesg(fit::UserProfileMesg &mesg) {
    printf("User profile:\n");
    if (mesg.IsFriendlyNameValid())
        std::wcout << L"   Friendly Name: " << mesg.GetFriendlyName().c_str()
                   << L"\n";
...
void FitFileListener::OnMesg(fit::DeviceInfoMesg &mesg) {
    printf("Device info:\n");
...
void FitFileListener::OnMesg(fit::MonitoringMesg &mesg) {
    printf("Monitoring:\n");
...
static void PrintOverrideValues(const fit::Mesg &mesg, FIT_UINT8 fieldNum) {
    std::vector<const fit::FieldBase *> fields = mesg.GetOverrideFields(
            fieldNum);
    const fit::Profile::FIELD *profileField = fit::Profile::GetField(
            mesg.GetNum(), fieldNum);
...
void FitFileListener::OnMesg(fit::RecordMesg &record) {
    printf("Record:\n");
